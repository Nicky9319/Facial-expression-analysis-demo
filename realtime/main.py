...
TARGET_FPS = 10  # realistic for CPU; on GPU this can be higher
        ...
        if face_mesh is not None:
            try:
                ...
                if results.face_landmarks and len(results.face_landmarks) > 0:
                    ...
                    symmetry = 1.0 - min(1.0,
                        abs(abs(nose_x - left_eye_x) - abs(right_eye_x - nose_x)) / eye_distance
                    )
...
def process_frame(frame, models, state, device):
...
def main():
    ...
    log("Loading YOLO...")
    models["yolo"], device = load_yolo()

    log("Loading emotion classifier...")
    models["emotion"], _ = load_emotion_classifier()

    log("Loading CLIP model...")
    models["clip_model"], models["clip_processor"] = load_clip()

    log("Loading MediaPipe face mesh...")
    models["face_mesh"] = load_face_mesh()
    ...
    while True:
        ...
        # ---- PROCESS ----
        frame = process_frame(frame, models, state, device)
